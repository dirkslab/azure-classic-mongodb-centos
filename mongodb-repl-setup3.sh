...
mongodump --gzip --out /data_disk/mongo/backups/mongodumps/mongodump_$(date +%Y%m%d-%H%H%M)' > /etc/scripts/mongodump.sh
...
echo '* 2 * * * root /etc/scripts/mongodump.sh' >> /etc/crontab
#echo '5 0 * * * /etc/scripts/mongologcleanup.sh'>/etc/scripts/mongologcleanup.txt
